      ...
      <section className="content-surface">
        <QueryFilterBar query={action} onQueryChange={setAction} placeholder={t('audit.searchAction')}>
          <Select
            ...
            value={result}
            onChange={setResult}
            options={[{ value: 'SUCCESS', label: t('status.success') }, { value: 'FAILED', label: t('status.failed') }]}
          />
        ...
        {resource.status === 'ready' ? (
          <DataTable
            ...
            columns={[
              ...
              { key: 'actor', title: t('audit.actor'), render: (row) => row.actorDisplayName },
              { key: 'action', title: t('audit.action'), render: (row) => <Link to={`/access/audit/${row.eventId}`}>{row.action}</Link> },
              { key: 'resource', title: t('audit.resource'), render: (row) => `${row.resourceType}${row.resourceId ? ` · ${row.resourceId}` : ''}` },
