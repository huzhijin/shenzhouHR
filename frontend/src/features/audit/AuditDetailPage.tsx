...
export function AuditDetailPage() {
  ...
  return (
    <>
      <PageHeader title={auditActionLabel(event.action)} description={t('audit.readOnlyDescription')} breadcrumbs={[{ label: t('access.section') }, { label: t('access.audit'), path: '/access/audit' }, { label: event.eventId }]} />
      <section className="content-surface">
        <ReadOnlyDetails items={[
          ...
          { label: t('audit.resource'), value: `${auditResourceLabel(event.resourceType)}${event.resourceId ? ` · ${event.resourceId}` : ''}` },
      ...
      {event.changes?.length ? (
        <section className="content-surface section-spaced">
          ...
          <AuditTimeline entries={Array.from(event.changes, (change) => ({
            ...
            result: event.result,
            detail: `${change.before ? statusLabel(change.before) : t('common.none')} → ${change.after ? statusLabel(change.after) : t('common.none')}`,
          }))} />
        </section>
      ) : null}
    </>
