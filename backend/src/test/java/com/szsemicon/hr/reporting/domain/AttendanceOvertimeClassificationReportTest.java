...
class AttendanceOvertimeClassificationReportTest {
    ...
    @Test
    void paid_and_compensatory_documents_are_separate_rows() {
        var report = calculator.calculate(
                ReportType.OVERTIME,
                documents(List.of(
                        overtime("paid", "PAID", 120),
                        overtime("compensatory", "COMPENSATORY", 60))));

        assertThat(report.columns())
                .extracting(column -> column.field().label())
                .contains("认定小时", "来源")
                .doesNotContain("部门加班汇总");
        assertThat(report.rows()).hasSize(2);
        assertThat(report.rows())
                .anySatisfy(row -> assertThat(row.values())
                        .containsEntry(ReportField.DOCUMENT_TYPE, "PAID")
                        .containsEntry(ReportField.RECOGNIZED_HOURS, "2.00"));
        assertThat(report.rows())
                .anySatisfy(row -> assertThat(row.values())
                        .containsEntry(ReportField.DOCUMENT_TYPE, "COMPENSATORY")
                        .containsEntry(ReportField.RECOGNIZED_HOURS, "1.00"));
    }
    ...
    private ReportSourceSnapshot documents(List<OaDocumentFact> facts) {
    ...
    private OaDocumentFact overtime(String id, String type, long minutes) {
        return new OaDocumentFact(
                ...
                "OVERTIME",
                type,
                Instant.parse("2026-07-15T09:40:00Z"),
