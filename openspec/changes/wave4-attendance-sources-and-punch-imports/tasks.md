...
- [x] 2.6 Add evidence interval slice, recalculation intent, evidence-subject lock and durable ingestion-idempotency tables with deterministic key/index contracts.
...
- [x] 3.1 Implement stable source/configuration aggregates and service ports using repository-external secret references without storing or returning credential values.
- [x] 3.2 Implement `DeliPunchSourcePort` canonical pages and a deterministic synthetic adapter covering long IDs, timezone, device/location evidence and forbidden biometric payloads.
- [x] 3.3 Implement `OaAttendanceDocumentSourcePort` canonical pages and versioned status/type mapping for approved, draft, rejected, unknown, modified, supplemented and revoked documents.
...
- [x] 3.8 Prove adapters expose no organization sync, upstream write-back, source delete or biometric-template persistence operation.
- [x] 3.9 Emit separate contract-stub and live-integration status fields so fixture PASS can never be relabeled as real 得力/OA联调 PASS.
...
- [x] 5.1 Add `V9__attendance_punch_import.sql` with immutable mapping versions, batch/file/row/attempt/issue/precheck/publication/state/error-report objects and the independent registry's constraints/indexes.
...
- [x] 5.9 Implement the exact append-only batch state machine with distinct `VALIDATION_FAILED`, `BLOCKED_BY_FROZEN_PERIOD` and `PUBLISH_FAILED` transitions and retry prerequisites.
...
- [x] 6.8 Implement `StoredObjectPort` and `MalwareScanPort` contracts plus fail-closed local/test adapters using opaque refs, safe paths and repository-external storage.
...
- [x] 7.2 Register `/sources/online`, `/sources/oa`, `/sources/jobs`, `/sources/attendance-excel` and `/sources/attendance-excel/:batchId` with capability-based route/menu/breadcrumb/redirect and real 404 behavior.
...
- [x] 7.11 Add an explicit synthetic demo adapter that short-circuits before API, separate prod/demo builds, visible demo labeling and no production import of demo datasets.
...
- [x] 8.3 Pass frontend typecheck, lint and tests as separate commands, including route, API, state, permission, accessibility and runtime-mode tests.
- [x] 8.4 Pass separate production and demo builds with newer-than-source inventories and production demo-dataset absence.
