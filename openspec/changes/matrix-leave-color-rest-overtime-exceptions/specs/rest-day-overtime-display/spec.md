...
A single punch on a rest day SHALL occupy the morning band when its local time is at or before 12:00, otherwise the afternoon band. A first and last punch SHALL occupy morning and afternoon bands respectively.

#### Scenario: Saturday overtime day with two punches
...
When a rest day has an effective approved OA overtime document covering that calendar date:

- If punch times are shown, those slots SHALL use overtime legend color and hover SHALL name 加班. Times stay on the grid; the word 加班 MUST NOT replace the times.
...
Weekday overtime color still requires recognized overtime minutes plus an effective document. Rest-day visibility MUST NOT depend on weekday shift segments existing.
