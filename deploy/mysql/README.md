...
`all` 会独立验证“含业务关系的升级路径”和“近空库路径”。两条路径都先停在
V6，显式建立固定 baseline company
`30000000-0000-0000-0000-000000000001`，不会在固定公司不存在时偷偷选择其他
公司；随后迁移到精确 V10，先校验 29 张边界表、29 个公司列、28 张依赖表上的
29 条精确关系边、5 组索引、范围 CHECK、孤儿记录和在途事务，再允许执行 V11。
含数据路径还会比较 V10、V11 和重复 migrate 后的行数、稳定 ID 与公司关系摘要。
V11 后最终结构及权限值统一为公司命名。每个运行态 PASS marker 都携带同一个
数据库身份，防止不同实例或不同 test schema 的证据被拼接。
