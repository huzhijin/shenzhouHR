...
--   * run on MySQL 8.4.10 after successful Flyway V31 (V31 is DDL-only);
...
main: BEGIN
    ...
    IF DATABASE() IS NULL
       OR SUBSTRING_INDEX(VERSION(), '-', 1) <> '8.4.10' THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'requires an explicitly selected schema on MySQL 8.4.10';
    END IF;
