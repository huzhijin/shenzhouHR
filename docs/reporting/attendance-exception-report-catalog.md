...
接口：`GET /api/v1/attendance-reports?reportType=EXCEPTIONS&period=YYYY-MM&companyId=...`
...
3. 公司、当前组织树、员工当前有效任职、部门负责人范围和本人范围均在 SQL 层先过滤，再进入聚合、计数和分页；发生时组织仅用于事实展示与汇总，不扩大负责人当前可见人员集合。
