...
调用按公司、自然月串行并在一个数据库事务中完成：

1. 校验并规范化投影 metadata、计算事实、OA 事实和时间账户事实；
2. 锁定公司行；
3. 按服务端 canonical 内容生成 `projection_digest` 与 `projection_version`；
4. 同公司、月份、摘要已存在且为 `PUBLISHED` 时幂等返回；
5. 锁内读取该公司、月份最新的已发布投影，拒绝更旧的 `dataAsOf` 和非法期间状态转换；
6. 写入 `DRAFT`；
...
- 日事实、异常事实的人员版本、任职、组织版本和公司会按 `business_date` 在写入 SQL 中形成唯一有效链。OA、时间账户目前只形成 ID/所属关系链，不等同于时点有效性校验。任一实际执行的事实写入行数不是 1 时，发布失败并回滚。
...
因此，本文件不声称“月结后已自动生成报表”。完成上游后，接线位置应是结算编排器确认同一公司和月份的计算版本、OA 来源版本、账本版本与 period state 均已锁定之后，调用一次 `AttendanceReportProjectionPublicationUseCase`。不得从查询控制器、导出控制器、浏览器定时器或得力同步分页事务直接调用。
...
3. `employee_version` 的 `DATE` 有效期与任职/组织版本的 `DATETIME(6)` 有效期如何在公司员工跨时区场景中比较；
